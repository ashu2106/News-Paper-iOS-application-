...
- (void)loadTutorials {
    // 1
    NSURL *tutorialsUrl = [NSURL URLWithString:@"http://www.lokmat.com/pune-1"];
    NSData *tutorialsHtmlData = [NSData dataWithContentsOfURL:tutorialsUrl];
    ...
    // 3
    NSString *tutorialsXpathQueryString_headline = @"//div[@class='article-section']/h2/a";
    NSArray *tutorialsNodes_headline = [tutorialsParser searchWithXPathQuery:tutorialsXpathQueryString_headline];
   
    NSString *tutorialsXpathQueryString_description = @"//div[@class='article-section']/h4";
    NSArray *tutorialsNodes_description = [tutorialsParser searchWithXPathQuery:tutorialsXpathQueryString_description];
    
    
    // 4
    NSMutableArray *newTutorials = [[NSMutableArray alloc] initWithCapacity:0];
    TFHppleElement *element_title;
    TFHppleElement *element_description;
    for (int i =0 ; i<tutorialsNodes_headline.count; i++)
    //for (TFHppleElement *element in tutorialsNodes )
    {
        // 5
        element_title = tutorialsNodes_headline[i];
        element_description = tutorialsNodes_description[i];
        
        Tutorial *tutorial = [[Tutorial alloc] init];
        [newTutorials addObject:tutorial];
        
        // 6
        tutorial.title = [[element_title firstChild] content];
        
        // 7
        tutorial.url = [element_title objectForKey:@"href"];
        
        
        tutorial.description = [[element_description firstChild] content];
...
- (UITableViewCell *)tableView:(UITableView *)tableView cellForRowAtIndexPath:(NSIndexPath *)indexPath {
    ...
    if (indexPath.section == 0) {
        ...
        cell.detailTextLabel.text = (thisTutorial.description);
